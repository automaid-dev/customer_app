...
import 'bag_purchase_screen.dart';
              ...
              children: [
                ...
                FilledButton.icon(
                  ...
                  onPressed: () => Navigator.of(context)
                      .push(MaterialPageRoute(builder: (_) => const BagPurchaseScreen())),
                ),
              ],
            ),
...
class _BagCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final orderId = bag['order_id'] as int?;
    final paymentStatus = bag['status_payment']?.toString();
    ...
    return Card(
      ...
      child: ListTile(
        leading: Icon(
          isPendingPayment ? Icons.hourglass_top_outlined : Icons.shopping_bag_outlined,
          color: isPendingPayment ? Colors.orange : null,
        ),
        title: Text('Bag #${bag['id']}'),
        subtitle: Text(
          isPendingPayment
              ? 'Awaiting payment confirmation · Qty: ${bag['quantity'] ?? 1}'
              : 'Status: ${bag['status'] ?? '-'} · Qty: ${bag['quantity'] ?? 1}',
          style: isPendingPayment ? const TextStyle(color: Colors.orange) : null,
        ),
        trailing: orderId != null ? const Icon(Icons.receipt_long_outlined) : null,
        onTap: orderId == null
            ? null
